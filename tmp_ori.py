...
def test_llama_verification_with_kv_cache_relocation():
    ...
    with torch.inference_mode():
        ...
        # print(f"save_gen_logits_0.shape: {save_gen_logits_0.shape} and save_gen_logits_0.max(): {save_gen_logits_0.max()} and save_gen_logits_0.min(): {save_gen_logits_0.min()} and save_gen_logits_0.mean(): {save_gen_logits_0.mean()}")
    ...
    attn_metadata_gen_phase_0.update_spec_dec_param(
        is_spec_decoding_enabled=is_spec_decoding_enabled,
        is_spec_dec_tree=is_spec_dec_tree
        if get_sm_version() < 100 else False,  #False for b100
    ...
    attn_metadata_ref.update_spec_dec_param(
        is_spec_decoding_enabled=is_spec_decoding_enabled,
        is_spec_dec_tree=is_spec_dec_tree
        if get_sm_version() < 100 else False,  #is_spec_dec_tree
        is_spec_dec_dynamic_tree=False,
    ...
    # Helper function to compute and print differences
    def print_difference_stats(actual, expected, token_name):
        print(f"\n{token_name} 差异统计:")
        print("-" * 60)

        # 基本统计
        print(
            f"实际值 (gen_logits_1): min={actual.min():.6f}, max={actual.max():.6f}, mean={actual.mean():.6f}"
        )
        print(
            f"期望值 (gen_logits_ref): min={expected.min():.6f}, max={expected.max():.6f}, mean={expected.mean():.6f}"
        )

        # 检查 NaN 和 Inf
        if torch.isnan(actual).any():
            print(f"⚠️  实际值包含 NaN: {torch.isnan(actual).sum()} 个")
        if torch.isinf(actual).any():
            print(f"⚠️  实际值包含 Inf: {torch.isinf(actual).sum()} 个")
        if torch.isnan(expected).any():
            print(f"⚠️  期望值包含 NaN: {torch.isnan(expected).sum()} 个")
        if torch.isinf(expected).any():
            print(f"⚠️  期望值包含 Inf: {torch.isinf(expected).sum()} 个")
        ...
        print(f"\n绝对差异:")
        print(
            f"  Greatest absolute difference: {max_abs_diff:.6f} at index ({max_abs_idx.item()},)"
        )
        print(f"  Mean absolute difference: {abs_diff.mean():.6f}")
        print(f"  Median absolute difference: {abs_diff.median():.6f}")

        # 相对差异（使用更合理的计算方式）
        # 方法1: 使用 max(abs(expected), abs(actual)) 作为分母，更对称
        denominator = torch.maximum(expected.abs(), actual.abs())
        # 只对显著值计算相对差异（避免小数值夸大）
        significant_mask = denominator > 1e-3  # 只考虑绝对值 > 0.001 的元素
        ...
        if significant_mask.any():
            ...
            max_rel_idx_sig = significant_indices[rel_diff_significant.argmax()]

            print(f"\n相对差异 (仅对显著值 |value| > 0.001):")
            print(
                f"  显著元素数量: {significant_mask.sum().item()} / {len(actual)} ({100*significant_mask.sum().item()/len(actual):.1f}%)"
            )
            print(
                f"  Greatest relative difference: {max_rel_diff_sig:.6f} at index ({max_rel_idx_sig.item()},)"
            )
            print(
                f"  Mean relative difference: {rel_diff_significant.mean():.6f}"
            )
            print(
                f"  Median relative difference: {rel_diff_significant.median():.6f}"
            )
        else:
            print(f"\n相对差异:")
        ...
        # 传统相对差异（用于对比，会被小值夸大）
        rel_diff_traditional = abs_diff / (expected.abs() + 1e-8)
        max_rel_diff_trad = rel_diff_traditional.max()
        max_rel_idx_trad = rel_diff_traditional.argmax()

        print(f"\n传统相对差异 (可能被小值夸大):")
        print(
            f"  Greatest: {max_rel_diff_trad:.6f} at index ({max_rel_idx_trad.item()},)"
        )
        print(
            f"    → 期望值: {expected[max_rel_idx_trad]:.6e}, 实际值: {actual[max_rel_idx_trad]:.6e}"
        )

        # 显示最大差异位置的具体值
        print(f"\n最大绝对差异位置 (index {max_abs_idx.item()}):")
        print(f"  实际值: {actual[max_abs_idx]:.6f}")
        print(f"  期望值: {expected[max_abs_idx]:.6f}")
        print(f"  绝对差异: {abs_diff[max_abs_idx]:.6f}")
        if denominator[max_abs_idx] > 1e-3:
            rel_at_max_abs = abs_diff[max_abs_idx] / (denominator[max_abs_idx] +
                                                      1e-8)
            print(f"  相对差异: {rel_at_max_abs:.6f} ({rel_at_max_abs*100:.2f}%)")

        if significant_mask.any():
            print(f"\n最大相对差异位置 (index {max_rel_idx_sig.item()}, 仅显著值):")
            print(f"  实际值: {actual[max_rel_idx_sig]:.6f}")
            print(f"  期望值: {expected[max_rel_idx_sig]:.6f}")
            print(f"  绝对差异: {abs_diff[max_rel_idx_sig]:.6f}")
            print(
                f"  相对差异: {max_rel_diff_sig:.6f} ({max_rel_diff_sig*100:.2f}%)")

        # 显示差异最大的前10个位置
        print(f"\n绝对差异最大的前10个位置:")
        top_indices = abs_diff.topk(10).indices
        for i, idx in enumerate(top_indices):
            denom = torch.maximum(expected[idx].abs(), actual[idx].abs())
            if denom > 1e-3:
                rel_pct = (abs_diff[idx] / (denom + 1e-8) * 100).item()
                print(
                    f"  {i+1}. Index {idx.item()}: actual={actual[idx]:.6f}, expected={expected[idx]:.6f}, "
                    f"diff={abs_diff[idx]:.6f} ({rel_pct:.1f}%)")
            else:
                print(
                    f"  {i+1}. Index {idx.item()}: actual={actual[idx]:.6f}, expected={expected[idx]:.6f}, "
                    f"diff={abs_diff[idx]:.6f} (值太小)")

        # 统计不同误差范围的元素数量
        print(f"\n误差分布:")
        total = len(actual)
        within_1pct = (abs_diff <= denominator * 0.01).sum().item()
        within_5pct = (abs_diff <= denominator * 0.05).sum().item()
        within_10pct = (abs_diff <= denominator * 0.10).sum().item()
        within_atol = (abs_diff <= 0.02).sum().item()

        print(
            f"  误差 ≤ 1% : {within_1pct:6d} / {total} ({100*within_1pct/total:.1f}%)"
        )
        print(
            f"  误差 ≤ 5% : {within_5pct:6d} / {total} ({100*within_5pct/total:.1f}%)"
        )
        print(
            f"  误差 ≤ 10%: {within_10pct:6d} / {total} ({100*within_10pct/total:.1f}%)"
        )
        print(
            f"  误差 ≤ 0.02 (atol): {within_atol:6d} / {total} ({100*within_atol/total:.1f}%)"
        )

        print("-" * 60)

    # Token 0 验证
    print_difference_stats(gen_logits_1[0, :], gen_logits_ref[2, :], "Token 0")
    ...
    # Token 1 验证
    print_difference_stats(gen_logits_1[1, :], gen_logits_ref[3, :], "Token 1")
