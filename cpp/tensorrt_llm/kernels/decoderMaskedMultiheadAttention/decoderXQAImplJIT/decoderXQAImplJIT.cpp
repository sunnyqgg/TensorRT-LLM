    ...
    int multiprocessor_count, cudaStream_t const& stream) const
{
            ...
            if (xqaParams.multi_query_tokens)
            {
                ...
                if constexpr (std::is_same_v<KVCacheBuffer, KVBlockArray>)
                {
                    ...
                    for (int token_idx = 0; token_idx < seq_len_host && k_idx < kv_print_size; token_idx++)
                    {
                        ...
                        cudaMemcpy(all_k_values.data() + k_idx, k_src, copy_size * sizeof(T), cudaMemcpyDeviceToHost);
                        k_idx += copy_size;
                    }
                    cudaDeviceSynchronize();
                    ...
                    for (int token_idx = 0; token_idx < seq_len_host && v_idx < kv_print_size; token_idx++)
                    {
                        ...
                        cudaMemcpy(all_v_values.data() + v_idx, v_src, copy_size * sizeof(T), cudaMemcpyDeviceToHost);
                        v_idx += copy_size;
                    }
                    cudaDeviceSynchronize();
    ...
    else if (isSpecDec && isHMMAKernel)
    {
        ...
        {
            int seq_len_val;
            cudaMemcpy(&seq_len_val, launchParams.kvCacheParams.sequence_lengths, sizeof(int), cudaMemcpyDeviceToHost);
            printf("[DEBUG-XQA-PARAMS] kvCacheParams.sequence_lengths[0]=%d\n", seq_len_val);
        }
