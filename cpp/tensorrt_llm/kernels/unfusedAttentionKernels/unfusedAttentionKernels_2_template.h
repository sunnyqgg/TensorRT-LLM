...
__global__ void applyBiasRopeUpdateKVCache(QKVPreprocessingParams<T, KVCacheBuffer> params)
{
    ...
    int local_token_idx = blockIdx.x * blockDim.y + threadIdx.y;
    {
        ...
        for (int batch_idx = blockIdx.z; batch_idx < params.batch_size; batch_idx += gridDim.z)
        {
            ...
            //         global_token_idx, batch_idx, local_token_idx, token_idx_in_seq, offset_idx,
            //         position_offset_value, past_seq_len, cache_seq_len, actual_seq_len, rotary_position, (int)
            //         valid_token);
            // }
...
__global__ void applyBiasRopeUpdateKVCacheV2(QKVPreprocessingParams<T, KVCacheBuffer> params)
{
         ...
         global_token_idx < tokens_loop_end; global_token_idx += TOKENS_PER_BLOCK * gridDim.x)
    {
        ...
        else if (variable_sequence_length)
        {
            ...
            //         bounded_global_token_idx, token_info.x, token_info.y, batch_idx, token_idx_in_seq, blockIdx.x,
            //         threadIdx.x);
            // }
        }
        else
        {
            ...
            // {
            //     printf(
            //         "[DEBUG-V2-INDEX-FIXED_LEN] global_token_idx=%d, bounded_global_token_idx=%d,
            //         max_input_seq_len=%d "
            //         "-> batch_idx=%d, token_idx_in_seq=%d\n",
            //         global_token_idx, bounded_global_token_idx, params.max_input_seq_len, batch_idx,
            //         token_idx_in_seq);
            // }
        }
        ...
        //     int const position_offset_value = params.spec_decoding_position_offsets[offset_idx];
        //     // printf(
        //     //     "[DEBUG-applyBiasRopeUpdateKVCacheV2] global_token_idx=%d, bounded_global_token_idx=%d,
        //     batch_idx=%d, "
        ...
        //     //     position_offset_value, cache_seq_len, actual_seq_len, past_seq_len, rotary_position, (int)
        //     GEN_PHASE,
        ...
        if (valid_token)
        {
            ...
            else
            {
                *q_ptr = q;
                ...
                //         global_token_idx, batch_idx, (int) STORE_QKV, (void*) q_ptr,
                //         float(reinterpret_cast<T*>(&q)[0]), float(reinterpret_cast<T*>(&q)[1]),
                //         float(reinterpret_cast<T*>(&q)[2]), float(reinterpret_cast<T*>(&q)[3]));
                // }
            }
