...
class Eagle3OneModelWorker(SpecWorkerBase):
    ...
    def _forward_linear_draft_loop(self, inputs, attn_metadata, spec_metadata,
        ...
        return torch.stack(next_draft_tokens, dim=1)
    ...
    def sample_and_accept_draft_tokens(
        ...
        draft_tokens = spec_metadata.draft_tokens.reshape(
            num_gens, self.max_draft_len) if num_gens > 0 else torch.empty(
                0, self.max_draft_len, dtype=torch.int, device=logits.device)
