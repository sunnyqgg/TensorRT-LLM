...
class Eagle3OneModelSampler(MTPSampler):
    """Sampler for one-model EAGLE3 linear tree mode."""
...
class Eagle3OneModelWorker(SpecWorkerBase):
    ...
    @nvtx_range("eagle3.prepare_attn_meta_for_spec_dec")
    def _prepare_attn_metadata_for_spec_dec(self, attn_metadata):
        ...
        if attn_metadata.spec_decoding_position_offsets is not None:
            ...
            self._saved_position_offsets_query_len = getattr(
                attn_metadata, 'position_offsets_query_len', 0)
        else:
            self._saved_position_offsets = None
            self._saved_position_offsets_stride = None
            self._saved_position_offsets_query_len = None
    ...
    @nvtx_range("eagle3.restore_attn_meta_from_spec_dec")
    def _restore_attn_metadata_from_spec_dec(self, attn_metadata):
        ...
        if getattr(self, '_saved_position_offsets_stride', None) is not None:
            ...
            self._saved_position_offsets_stride = None
        if getattr(self, '_saved_position_offsets_query_len', None) is not None:
            attn_metadata.position_offsets_query_len = (
                self._saved_position_offsets_query_len)
            self._saved_position_offsets_query_len = None
    ...
    def forward(self,
        ...
        with nvtx_range("eagle3.sample_and_accept"):
            accepted_tokens, num_accepted_tokens = self.sample_and_accept_draft_tokens(
                logits, attn_metadata, spec_metadata)

        with nvtx_range("eagle3.sa_enhance"):
            sa_manager = getattr(spec_metadata.spec_resource_manager,
                                 'sa_manager', None)
        ...
        with nvtx_range("eagle3.save_attn_metadata"):
            self._prepare_attn_metadata_for_spec_dec(attn_metadata)

        # Prepare inputs for the 1st draft model forward
        with nvtx_range("eagle3.prepare_1st_drafter_inputs"):
        ...
        with nvtx_range("eagle3.draft_loop"):
        ...
        with nvtx_range("eagle3.restore_attn_metadata"):
        ...
        with nvtx_range("eagle3.prepare_next_new_tokens"):
    ...
    @nvtx_range("eagle3.linear_draft_loop")
    def _forward_linear_draft_loop(self, inputs, attn_metadata, spec_metadata,
        ...
        with self.draft_kv_cache_context(attn_metadata, draft_kv_cache_manager):
            for i in range(runtime_draft_len):
                with nvtx_range(f"eagle3.draft_step_{i}"):
                    with nvtx_range(
                            f"eagle3.draft_step_{i}.prepare_gather_ids"):
                        if i == 0:
                            num_gens = batch_size - num_contexts
                            start_ids_gen = (
                            ...
                            gather_ids = torch.concat([
                                spec_metadata.gather_ids[:num_contexts],
                                gather_ids_gen
                            ],
                                                      dim=0)
                        else:
                    ...
                    with nvtx_range(f"eagle3.draft_step_{i}.draft_model_fwd"):
                    ...
                    with nvtx_range(f"eagle3.draft_step_{i}.logits_and_sample"):
                    ...
                    # update inputs
                    with nvtx_range(f"eagle3.draft_step_{i}.update_inputs"):
                        hidden_states = hidden_states_to_save[gather_ids]
                        position_ids = inputs["position_ids"][gather_ids] + 1

                    # update attn_metadata
                    with nvtx_range(f"eagle3.draft_step_{i}.update_attn_meta"):
                        if i == 0:
        ...
        with nvtx_range("eagle3.stack_draft_tokens"):
        ...
        if self.sa_enhancer is not None:
            with nvtx_range("eagle3.sa_override"):
    ...
    @nvtx_range("eagle3.sample_and_accept_draft_tokens")
    ...
    @nvtx_range("eagle3.prepare_1st_drafter_inputs")
    def prepare_1st_drafter_inputs(
        ...
        with nvtx_range("eagle3.prepare_1st.eagle3_fc"):
        ...
        # context
        with nvtx_range("eagle3.prepare_1st.ctx_input_ids"):
            input_ids_ctx = self._prepare_context_input_ids(
                input_ids, attn_metadata.num_ctx_tokens,
                spec_metadata.gather_ids, accepted_tokens, num_contexts)

        # generation
        with nvtx_range("eagle3.prepare_1st.gen_input_ids"):
