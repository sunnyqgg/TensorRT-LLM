...
class DynamicTreeDraftingLoopWrapper(BaseDraftingLoopWrapper):
    ...
    def prepare_tree_mask_and_position_offset(
        ...
        if cur_draft_idx == 0:
            ...
            # Append K new positions by incrementing the last K positions by 1
    ...
    def update_hidden_states_indices(self,
                                     ...
                                     selected_parents: torch.Tensor = None):
        """
        Update hidden states read indices so the next draft layer reads the correct hidden states.

        Flow:
        1. Current layer (cur_draft_idx) inference is done, producing (cur_draft_idx+1)*K tokens.
        2. From the last K write_indices of the current layer, select entries via selected_parents.
        3. Update the next layer's read_indices to point to the selected parent hidden states.

        Args:
            cur_draft_idx: 0-based draft layer index of the just-completed layer.
            spec_metadata: Metadata containing hidden_states_read/write_indices.
            selected_parents: [batch_size, K], indices in [0, K-1] selecting which of the
                              last K tokens become parents for the next layer.
        """
        K = self.dynamic_tree_max_topK
        batch_size = attn_metadata.num_seqs
        if cur_draft_idx == 0:
            ...
            # 1. Compute the range of the last K tokens for previous/current layers.
            #    Layer i generates tokens at positions [i*K : (i+1)*K].
            ...
            # 2. Reshape hidden_states_write_indices into a 2D [batch_size, num_tokens] view
            ...
            # 3. Extract write_indices for the last K tokens of the previous layer
            last_K_write_indices = hidden_states_write_indices_view[:,
                                                                    last_K_start_for_previous_layer:
                                                                    last_K_end_for_previous_layer]
            # shape: [batch_size, K]

            # 4. Gather the selected parents from the last K to form next layer's read_indices
            ...
            # These are absolute positions in the global hidden_states buffer

            # 5. Update the next layer's read_indices.
            #    The first cur_draft_idx*K entries are kept from prior layers;
            #    the last K entries are set to the selected parent positions.
            ...
            # 6. Update write_indices for the next layer.
            #    Prior entries are unchanged; append K new indices starting from last_write_index + 1.
            ...
            # Get the last write_index from the previous layer
            last_write_index = hidden_states_write_indices_view[:,
                                                                -1]  # [batch_size]

            # Generate K new consecutive write_indices: [last+1, last+2, ..., last+K]
            new_write_indices = last_write_index.unsqueeze(1) + torch.arange(
                1, K + 1, device='cuda', dtype=torch.long).unsqueeze(
                    0)  # [batch_size, K]

            # Write the new indices into the last K positions of the current layer
